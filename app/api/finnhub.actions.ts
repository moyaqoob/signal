...
import { POPULAR_STOCK_SYMBOLS } from "@/lib/config";
...
export const AddFetchStockDetails = async (symbol: string) => {
  try {
    await connectToDatabase(); // ensure MongoDB is ready
    ...
    if (!profile?.name) {
      console.warn("No profile found for symbol:", symbol);
      return null;
    }
    ...
    const metricsUrl = `${FINNHUB_BASE_URL}/stock/metric?symbol=${symbol}&metric=all&token=${token}`;
    const metrics = await fetchJSON<any>(metricsUrl, 3600);
    const peRatio = metrics?.metric?.peNormalizedAnnual || 0;

    const existing = await WatchListModel.findOne({ symbol });
    if (existing) {
      console.log(`⚠️ ${symbol} already exists in watchlist.`);
      return existing;
    }
    console.log("user 2 before doc");
    const doc = await WatchListModel.create({
      ...
      price: quote?.c ?? 0,
      change: quote?.d ?? 0,
      marketCap: profile.marketCapitalization?.toString(),
      peRatio: peRatio,
      action: "Hold",
    });

    console.log(`✅ Added ${symbol} (${profile.name}) to watchlist ${session?.user.id}`, doc);
    return doc.toObject();
  } catch (err) {
    console.error("❌ Error fetching or saving stock data:", err);
    throw err;
  }
};
